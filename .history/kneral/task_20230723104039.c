...
struct task_t task_ready_ender;
struct task_t task_run_header;
struct task_t task_run_ender;
struct task_t task_sleep_header;
struct task_t task_sleep_ender;

int global_ticks = 0;
void schedule()
{
    interrupt_disalow();
    task_t *next_task,*cur_task;
    ...
    // if(next_task->stack <= 0xa000-70&&next_task==0x9000){
    //     while(1);
    // }
...
void check_timer()
{
    ...
    task = task_sleep_header.next_task;

    
    while(task!=&task_sleep_ender)
     {
        //print("l?l");
...
char tasks_order(int select, ...)
{
    ...
    case update_auto://自动更新
        ...
        break;
    case update_change://更新修改内容
        
        int * p = &select;

        //当前需要唤醒的任务
        task_t *task = tasks.tasks_pcb[*(p+3)],*mid;
        p+=1;
        switch (*p)
        {
        ...
        case READY:
            ...
            case RUNNING:
                ...
                mid = task->fond_task;
                mid ->next_task = task->next_task;
                mid = task->next_task;
                mid->fond_task = task->fond_task;

                mid = task_run_ender.fond_task;
                mid->next_task = task;
                task->next_task = &task_run_ender;
                task_run_ender.fond_task = task;
                break;
            case SLEEP:
                ...
                mid = task_sleep_ender.fond_task;
                mid->next_task = task;
                task->next_task = &task_sleep_ender;
                task_sleep_ender.fond_task = task;
            default:
                break;
            }
            break;
        ...
        case RUNNING:
            ...
            case READY:
                ...
                mid = task_ready_ender.fond_task;
                mid->next_task = task;
                task->next_task = &task_ready_ender;
                task_ready_ender.fond_task = task;
                break;
            case SLEEP:
...
{
    print("uu");
    int current_status = task->status;
    task->status = status;
    task->sleep_ticks = 0;
    tasks_order(update_change,current_status,status,task->gid);
