...
void schedule()
{
    ...
    //         if(tasks.tasks_pcb[i]->status==READY){
    //             sp= tasks.tasks_pcb[i]->stack;
    //             tasks.tasks_pcb[i]->status = RUNNING;
    ...
    // if(sp==0){
    //         for(int i=0;i<max_task;i++){
    //         if(tasks.tasks_pcb[i]->status==RUNNING){
    //             sp= tasks.tasks_pcb[i]->stack;
    //             tasks.tasks_pcb[i]->status = READY;
    ...
    // {
        if(mmu%2 ==0){
            sp = tasks.tasks_pcb[2]->stack;
        }
        else{
            sp = tasks.tasks_pcb[1]->stack;
        }
    //}
    mmu++;
    // else if(uuu%3 ==0){
...
char init_pcb(char ID, struct task_t *task_pcb, char *name, int *task_space)
{
    ...
    pcb->stack = (int)task_pcb + page_size - 64;
    pcb->status = READY;
    pcb->priority = 3;
    pcb->ticks = 10;
    pcb->jiffies = 100;
