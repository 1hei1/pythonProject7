            ...
            for(int i=0;i<max_task;i++)
            {
                switch(tasks.tasks_pcb[i]->status)
                {
                    ...
                    case READY:
                        ...
                        else if(task_ready_header.next_task!=&task_ready_ender)
                        {
                            task_t *task;
...
char init_pcb(char ID, struct task_t *task_pcb, char *name, int *task_space,int priority)
{
    struct task_t *pcb;
    pcb = task_pcb;
...
char creat_task(char *name, int *code,int priority)
{
    ...
    *last_task = task_pcb;
    ...
    // 初始化pcb
    init_pcb(last_task_id, task_pcb, name, task_space,priority);
